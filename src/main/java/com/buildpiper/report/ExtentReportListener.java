    ...
    private void buildTestNodes(ExtentTest extenttest,IResultMap tests, Status status) {
        ...
        if (tests.size() > 0) {
            ...
            for (ITestResult result : treeSet) {
                ...
                //RK
              //  System.out.println("---------------"+com.buildpiper.report.Log.saveScreenShot(Capability._session).toString());
                try {
					test.addScreenCaptureFromPath(com.buildpiper.report.Log.saveScreenShot(Capability._session).toString());
				} catch (IOException e) {
