...
//		ui_switchToNewWindow();
	...
	@FindBy(xpath = "//select[@name='BP_VERSIONING_IS_APPROVAL_ENABLE']")
	WebElement enableDisableVersioningApproval;
	...
	public SuperAdminPage BuildPiperVersioningDisable() {
		...
		Select dropdown1 = new Select(enableDisableVersioningApproval);
		dropdown1.selectByVisibleText("false");
		
		if(ui_IsElementPresent(BpConfigCheckboxchecked,"3")==true) {
		ui_click(BPConfigmapCheckbox, "BPConfigmap Checkbox");	
		}
		if(ui_IsElementPresent(BpSecretCheckboxchecked,"3")==true) {
		ui_click(BpSecretCheckbox, "BpSecret Checkbox");	
		}
	...
	@FindBy(xpath = "(//input[@name='BP_VERSIONING_APPROVAL_BP_KIND_LIST' and @value='BPConfigmap']//ancestor::span[contains(@class,'Mui-checked')])[1]")
	WebElement BpConfigCheckboxchecked;
	@FindBy(xpath = "(//input[@name='BP_VERSIONING_APPROVAL_BP_KIND_LIST' and @value='BPSecret']//ancestor::span[contains(@class,'Mui-checked')])[1]")
	WebElement BpSecretCheckboxchecked;
	@FindBy(xpath = "//input[@name='BP_VERSIONING_APPROVAL_BP_KIND_LIST' and @value='BPConfigmap']")
	WebElement BPConfigmapCheckbox;
	@FindBy(xpath = "//input[@name='BP_VERSIONING_APPROVAL_BP_KIND_LIST' and @value='BPSecret']")
	WebElement BpSecretCheckbox;
	public SuperAdminPage BuildPiperVersioningEnable() {
		...
		if(ui_IsElementPresent(BpConfigCheckboxchecked,"3")==false) {
		ui_click(BPConfigmapCheckbox, "BPConfigmap Checkbox");	
		}
		if(ui_IsElementPresent(BpSecretCheckboxchecked,"3")==false) {
		ui_click(BpSecretCheckbox, "BpSecret Checkbox");	
		}
		ui_click(ApplySettingImmediately, "clicks on Apply Setting Immediately button");
